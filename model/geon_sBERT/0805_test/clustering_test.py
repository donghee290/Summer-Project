...
plt.rcParams['font.family'] = ['AppleGothic', 'Malgun Gothic', 'Noto Sans CJK KR', 'Arial Unicode MS']
...
class KoSimCSENewsPipeline:
    ...
    def check_dependencies(self) -> bool:
        ...
        if missing_packages:
            print(f"❌ 다음 패키지가 필요합니다: {missing_packages}")
            print("pip install sentence-transformers pandas numpy scikit-learn hdbscan matplotlib seaborn tqdm")
            return False
    ...
    def load_news_data(self, file_path: str = None) -> bool:
        ...
        try:
            ...
            if 'news' not in news_data :
                print("❌ JSON에 'news' 키가 없습니다.")
                return False
            if not isinstance(news_data['news'], dict):
                print("❌ 'news' 키 값이 딕셔너리가 아닙니다.")
                return False
            ...
            for category, article_list in news_data['news'].items():
                print(f"DEBUG: 카테고리 {category}에 기사 {len(article_list)}건")
    ...
    def run_clustering(self) -> Dict:
        ...
        if self.cluster_labels is not None:
            ...
            print(f"DEBUG: 군집별 기사 수 = {dict(zip(unique, counts))}")
    ...
    def analyze_clusters(self) -> Dict:
        ...
        print(f"DEBUG: 전체 cluster_analysis 길이 = {len(analysis)}")
        if len(analysis) > 0:
            for k, v in analysis.items():
                print(f"DEBUG: 클러스터 {k}: {v['size']}개 기사")
        else:
            print("DEBUG: 군집 분석 결과가 비어 있습니다!")
        #self.cluster_analysis = analysis
        analysis_conv = {str(int(k)): v for k, v in analysis.items()}
        self.cluster_analysis = analysis_conv
        return analysis

    def calculate_metrics(self) -> Dict:
        print(f"DEBUG: cluster_labels 상태: {self.cluster_labels}")
        if self.cluster_labels is None:
            return {}
...
def main():
    ...
    print("DEBUG: cluster_analysis 저장 직전 내용 = ", pipeline.cluster_analysis)
    if success:
